...
export let print = id => (dispatch, getState) => {
    var self = this;
    var newState = {};
...
let resizeTable = (table, tableContainer) => {
    ...
    var spaceLeft = window.innerHeight - 40 * 2 - table.offsetTop - 40;
    tableContainer.style.maxHeight = spaceLeft + 'px';
}

export let removeTableViewEvents = () => (dispatch, getState) => {
    console.log('removed')
    $(window).off('resize.tableView');
}

export let addTableViewEvents = (table, tableContainer) => (dispatch, getState) => {
