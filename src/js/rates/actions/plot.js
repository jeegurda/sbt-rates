...
export let drawPlotByCodes = (codes, plots) => (dispatch, getState) => {
  ...
  codes.forEach(code => {
    let plot = {
      ...
      getTooltip(type) {
        return '<div class="plot-tooltip">' +
            `<h3>${state.settings.dict[`current${utils.capitalize(type)}`]}</h3>` +
            '<p>%s: <em>%s</em></p>' +
            '<p>' +
              `${state.settings.dict.plotMax} ` +
              `<em>${utils.format(plot.values.max[type])} ${state.settings.destinationCurrencyShort}</em>` +
              `, ${state.settings.dict.plotMin} ` +
              `<em>${utils.format(plot.values.min[type])} ${state.settings.destinationCurrencyShort}</em>` +
            '</p>' +
          '</div>';
      },
      ...
      getPlotData() {
        ...
        item.ratesDated.forEach(el => {
          if (el.rangeFrom === checkedRange) {
            if (plot.values.invalid(el.activeFrom, el.buyValue, el.sellValue)) {
            ...
            plot.values.fill('period', el.activeFrom);
            plot.values.fill('buy', el.buyValue);
            plot.values.fill('sell', el.sellValue);

            buyValues.push([ el.activeFrom, el.buyValue ]);
            sellValues.push([ el.activeFrom, el.sellValue ]);
          }
        });
        ...
        if (buyValues.length && sellValues.length) {
          [ buyValues, sellValues ].forEach(arr => {
            if (arr.length === 1) {
              ...
              arr[1][0] += 100;
              plot.values.max.period = plot.values.min.period + 100;
            }
          });
      ...
      init() {
        return $.jqplot(`plot-${code}`, plot.data, {
          grid: {
          ...
          axes: {
            xaxis: {
              ...
              min: plot.values.min.period,
              max: plot.values.max.period,
              numberTicks: 6
            },
            yaxis: {
              borderWidth: 0,
              ...
              tickOptions: {
                ...
                formatter: (format, value) => `${utils.format(value)} ${state.settings.destinationCurrencyShort}`
              }
            }
          },
          legend: {
          ...
          series: [
            {
              ...
              color: '#4ec42c',
              highlighter: { formatString: plot.getTooltip('buy') }
            },
            {
              color: '#ffa101',
              label: state.settings.dict.currentSell,
              highlighter: { formatString: plot.getTooltip('sell') }
            }
          ]
        });
      }
    ...
    $(`#plot-${code}`).bind('jqplotMouseMove', function(e, gridpos, datapos, neighbor) {
      if (!neighbor) {
        return;
      }
