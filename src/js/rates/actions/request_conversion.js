...
export let requestConversion = () => (dispatch, getState) => {
    ...
    let params = {
        ...
        fromCurrencyCode: state.converter.from, // could be an empty string
        toCurrencyCode: state.converter.to, // could be an empty string
...
export let requestConversionAndScroll = () => (dispatch, getState) => {
    dispatch( actions.requestConversion() )
        .then(() => {
            console.log('NYI: scrolling')
            // scrollToResult();
        });
};